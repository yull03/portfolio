...
const About = () => {
  ...
  const SkillImg = [
    ...
    { src: `${process.env.PUBLIC_URL}/images/js.png`, alt:"jsvascript" },
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.timeline({
        ...
        .to(".about-title .char", {
          ...
          ease: "back.out(1.7)",
          duration: 0.4,
          stagger: 0.05 // 글자 간 간격
        });
              // === 2) Skill 이미지: 섹션 보일 때 등장 → 이후 2초마다 랜덤 흔들림 반복 ===
      ...
      .call(() => {
        imgs.forEach((el) => {
          // 각 이미지 시작 타이밍 랜덤화 (0~2초)
          const startDelay = gsap.utils.random(0, 2, 0.1);

          // 1) 흔들림 0.25s → 복귀 0.25s → 쉬기 1.5s = 총 약 2s 주기
          gsap.timeline({
            repeat: -1,
            repeatDelay: 1.5,
